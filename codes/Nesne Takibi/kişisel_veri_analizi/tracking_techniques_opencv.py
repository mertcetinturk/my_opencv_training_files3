...
while True:
	...
	if ret:
		...
		if len(car_gt) != 0:
			x = car_gt.x.values[0]  # .x diyince seri olarak dönüyor .value diyince array haline getiriyor
									# 0 ıncı indeksi alınca da içindeki değerleri döndürüyor
			y = car_gt.y.values[0]
			w = car_gt.w.values[0]
			h = car_gt.h.values[0]
